...
class PlatformController extends Zend_Controller_Action {
    ...
    public function queryAction() {
        ...
        $sql = "select * from platform where projectid=${projectid} 
        	order by platformid";
    ...
    public function addAction() {
        ...
        $where = $this->db->quoteInto('pfname = ?', $_POST['pfname']);
        $countExist = $platform->fetchAll($where)->count();
        if ($countExist > 0) {
            echo 'platform_exist';
            return;
        }
        ...
        $data = array(
            'projectid' => $_POST['projectid'],
        	'pfname' => $_POST['pfname'],
        	'pfusage' => $_POST['pfusage'],
            'createdtime' => date('Y-m-d H:i:s')
        );
        $id = $platform->insert($data);
        if ($id > 0) {
            $message = 'Add platform successfully [ID=' . $id . ']';
            Logging::write(Logging::MODULE_PROJECT, Logging::LEVEL_INFO, $message);
            echo $id;
        } else {
            $message = 'Add platform unsuccessfully [ID=null]';
    ...
    public function deleteAction() {
        ...
        $platformRow = $platform->find($platformid)->current();
        
        $row = $platformRow->delete();
        if ($row == 1) {
            $message = 'Delete platform successfully [ID=' . $platformid . ']';
            Logging::write(Logging::MODULE_PROJECT, Logging::LEVEL_INFO, $message);
            echo $row;
        } else {
            $message = 'Delete platform unsuccessfully [ID=' . $platformid . ']';
            Logging::write(Logging::MODULE_PROJECT, Logging::LEVEL_ERROR, $message);
            echo 'error';
        }
    }
